...
std::shared_ptr<const Source> Source::buildInparam(int sindex) {
    ...
    // horizontal
    root += ":location";
    bool axial = false, sourceCentered = false;
    ...
    return std::make_shared<const Source>(sourceName, axial, sourceCentered,
...
computeSPZ(const SE_Model &sem, const eigen::DRow3 &crdIn,
           bool sourceCentered, bool useDepth, bool depthSolid,
           bool undulatedGeometry, const std::string &errInfo,
           bool enforceOnAxis) {
    // result
...
const eigen::DMat33 &Source::computeQzsp(const eigen::DRow3 &spz) const {
...
void Source::release(const SE_Model &sem, Domain &domain, double dt,
    ...
    for (int sindex = 0; sindex < sourceCount; sindex++) {
        ...
        computeSPZ(sem, source->mCrdIn, source->mSourceCentered,
    ...
    for (int sindex = 0; sindex < sourceCount; sindex++) {
        ...
        // release
        mechanism->release(source->computeQzsp(spz), source->mAxial,
                           inplaneFactor, spz(1), sem.getQuad(quadTag),
                           stf, domain);
    }
