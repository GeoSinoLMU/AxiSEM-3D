...
public:
    // constructor
    Source(const std::string &name, bool axial, bool sourceCentered,
           bool useDepth, bool depthSolid, bool undulatedGeometry,
           const eigen::DRow3 &crdIn):
    mName(name), mAxial(axial), mSourceCentered(sourceCentered),
...
private:
    ...
    const eigen::DMat33 &computeQzsp(const eigen::DRow3 &spz) const;
