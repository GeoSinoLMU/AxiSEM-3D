...
std::unique_ptr<ABC> ABC::buildInparam(const ExodusMesh &exodusMesh) {
    ...
    if (abc->mSponge) {
        ...
        // check values
        if (*std::min_element(relSpan.begin(), relSpan.end()) < 0. ||
            *std::max_element(relSpan.begin(), relSpan.end()) > 1.) {
            throw std::runtime_error("ABC::buildInparam || "
                                     "Kosloff_Kosloff:relative_spans "
                                     "must range between [0, 1].");
        }
        if (*std::min_element(U0.begin(), U0.end()) < 0. ||
            *std::max_element(U0.begin(), U0.end()) > 1.) {
