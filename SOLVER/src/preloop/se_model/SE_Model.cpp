...
measureCost(const ExodusMesh &exodusMesh, const LocalMesh &localMesh,
    ...
    std::vector<std::string> elemSignature;
    for (const Quad &quad: mQuads) {
        // get element ptr
        std::shared_ptr<const Element> elem = quad.getElement();
        // get cost signature
        const std::string &esign = elem->costSignature();
        elemSignature.push_back(esign);
        // create in library
        elemCostLibrary.insert(std::pair<std::string, double>(esign, -1.));
        ...
        int count = minCount;
        double measured = elem->measure(count);
        while (measured < minTime) {
            count *= 10;
            measured = elem->measure(count);
        }
        // store in library
    ...
    ////////// measure nodes //////////
    timer::gPreloopTimer.begin("Measuring node cost library");
    std::map<std::string, double> nodeCostLibrary;
    std::vector<std::string> nodeSignature;
    for (const GLLPoint &gll: mGLLPoints) {
        // get point ptrs
        std::shared_ptr<SolidPoint> sp = gll.getSolidPoint();
        std::shared_ptr<FluidPoint> fp = gll.getFluidPoint();
        // get cost signature
        std::string nsign;
        if (sp && fp) {
            // on solid-fluid boundary
            nsign = sp->costSignature() + "+" + fp->costSignature();
        } else {
            nsign = sp ? sp->costSignature() : fp->costSignature();
        }
        nodeSignature.push_back(nsign);
        // create in library
        nodeCostLibrary.insert(std::pair<std::string, double>(nsign, -1.));
        // perform measurement only if it is new (measured = -1.)
        if (nodeCostLibrary.at(nsign) > 0.) {
            continue;
        }
        // increase measured count until measured time > minimum time
        int count = minCount;
        using point_time::measure;
        double measured = ((sp ? measure(*sp, count, timeScheme) : 0.) +
                           (fp ? measure(*fp, count, timeScheme) : 0.));
        while (measured < minTime) {
            count *= 5;
            measured = ((sp ? measure(*sp, count, timeScheme) : 0.) +
                        (fp ? measure(*fp, count, timeScheme) : 0.));
        }
        // store in library
        nodeCostLibrary.at(nsign) = measured / count;
    }
    timer::gPreloopTimer.ended("Measuring node cost library");
    ...
    mpi::aggregate(nodeCostLibrary, 0, MPI_MIN);
...
int SE_Model::locateInplane(const eigen::DCol2 &sz, bool inFluid) const {
    ...
    // one node is shared by at most 6 elements
    const int nNear = 6;
    if (inFluid) {
